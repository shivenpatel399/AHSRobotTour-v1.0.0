...
void subLeftNew(int baseSpeed, int slowLimit) {
  ...
  while (float(yaw) > targetAngle) {
    ...
    display.println("Inside while loop");
    display.display();
  }
  thisT = micros();
  stop(); 
  ...
  display.println("Outside while loop");
  display.display();
...
void PDDecel(int thesteps, int baseSpeed, int slowLimit, float gain, float slowPercent) {
  ...
  // total combined counts robot must reach:
  int totalCounts = thesteps * 2;

  // slowdown starts when remaining counts < slowPercent * totalCounts
  float slowdownRangeCounts = slowPercent * totalCounts;

  while ((counter_A + counter_B) < totalCounts) {

    yaw = computeYaw();

    // remaining counts before stopping
    float countsRemaining = float(totalCounts - (counter_A + counter_B));
    if (countsRemaining < 0) countsRemaining = 0;

    // ----- DECELERATION BASE SPEED (same structure as turn function) -----
    float currentBase;

    if (countsRemaining < slowdownRangeCounts) {
      // inverted mapping:
      // remaining = 0    → slowLimit
      // remaining = range → baseSpeed
      currentBase = map(countsRemaining,
                        0,
                        slowdownRangeCounts,
                        slowLimit,
                        baseSpeed);
    ...
    float delta = gain * error;   // correction strength

    float leftF  = currentBase - delta;
    float rightF = currentBase + delta;

    int powerLeft  = constrain((int)leftF,  0, 255);
    int powerRight = constrain((int)rightF, 0, 255);
    ...
    display.print("Remain: "); display.println(countsRemaining);
    display.print("Base: "); display.println(currentBase);
    display.print("L: "); display.println(powerLeft);
    display.print("R: "); display.println(powerRight);
    display.display();
  }

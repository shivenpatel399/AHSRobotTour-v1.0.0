...
void speedTime_control(float Time) { // Critical Change Point
  ...
  powerneeded = 80; // 12.3 + (3.21 * distovertime) + (-0.0149 * pow(distovertime, 2));
  counts = 410; // 444 + (0.0333 * powerneeded) + ((-5.71 * pow(10,-3)) * pow(powerneeded,2));
  gain = 5; // 4.61 + (-1.32 * pow(10,-3) * powerneeded) + ((7.74 * pow(10,-5)) * (pow(powerneeded,2))); 
}
